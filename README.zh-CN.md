...
> **延迟说明(实测跨网络):** 在同一 WiFi/局域网下很跟手;**换到不同网络**(另一个 WiFi、蜂窝数据)响应会明显变慢——如果流量还走了中继就更慢(例如本机开着 Clash 这类全局代理 TUN 模式,Tailscale 直连打洞被劫持、退回 DERP 中继)。这只影响**控制通道**(发消息、屏幕刷新)的手感,**不影响 agent 在机器上实际执行的速度**。聊天用了乐观本地回显来缓解卡顿感。
